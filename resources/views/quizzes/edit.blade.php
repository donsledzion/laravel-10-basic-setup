...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ ucfirst(__('quiz.edit.title')) }}: <i>"{{ $quiz->scenario->name }}"</i></div>
                ...
                <div class="card-body">
                    ...
                    <form id="quiz_form" method="POST" enctype="multipart/form-data" action="{{ route('quiz.update',[$quiz]) }}">
                        @csrf                        
                        @method("PATCH")
                        <div class="col">
                            <div class="row">
                                ...
                                <div class="col-10">
                                    <select id="type" name="type" class="form-select mx-2" disabled>
                                        @foreach(\App\Enums\QuizTypes::cases() as $type)
                                            <option value="{{ $type->value }}" @if( old('type') == $type->value || $quiz->type == $type) selected @endif>{{ ucfirst(__('quiz.types.'.$type->value)) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <input type="text" id="question_text" name="question_text" class="form-control" value="{{ old('question_text',$quiz->question_text) }}"  />
                                    @error('question_text')
